...
const departmentget = async function(req,res){
    try {
       ...
       await client.query(`select d.department_id, d.department_name,  array_agg(jsonb_build_object( d.department_id , t.team_name)) as Teams from team t join department d on t.department_id = d.department_id group by d.department_id`,function(err,result){
            if(err){
