...
const candidateget = async function(req,res){
   ...
   await   client.query(`SELECT * FROM "candidate"`,function(err,result){
          if(err){
  ...
  const candidateinsert = async function(req,res){
    try {
     ...
     await  client.query(`INSERT INTO "candidate" (candidate_name,email,phone,urls,prev_company) VALUES($1,$2,$3,$4,$5)`,[candidate_name,email,phone,urls,prev_company],function(err,result){
          if(err){
              console.log(err.message);
          }
          else{
  ...
  const candidategetbyid = async function(req,res){
    ...
    await client.query(`SELECT * FROM "candidate" WHERE candidate_id=$1 `,[candidate_id],function(error,result){
        if(!error){
            res.json({
                data:result.rows,
            })
