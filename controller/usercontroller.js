...
const userget = async function(req,res){
    try {
       ...
       await client.query(`SELECT u.user_id,u.user_name,u.email,r.role_name FROM "users" u join "roles" r on u.role_id=r.role_id`,function(err,result){
            if(err){
 ...
 const userinsert = async function(req,res){
    try {
      ...
      await client.query(`INSERT INTO "users" (user_name,company_id,department_id,role_id,email,password) VALUES($1,$2,$3,$4,$5,$6)`,[user_name,company_id,department_id,role_id,email,password],function(err,result){
          if(err){
              console.log(err.message);
          }
          else{
  ...
  const usergetbyid = async function(req,res){
    ...
    await client.query(`SELECT * FROM "users" WHERE user_id=$1 `,[user_id],function(error,result){
        if(!error){
            res.json({
                data:result.rows,
            })
...
const userdeleted = async function(req,res){
    try {
      //let query = "SELECT * FROM company";
      let {user_id} = req.params
     await client.query(`Delete From "users" WHERE user_id=$1`,[user_id],function(error,result){
          if(!error){
           res.json({
               msg:"Data Deleted",
           })
  ...
  ///
  module.exports = {
      userinsert,usergetbyid,userdeleted,userget,userupdated,usergetbyrole
  }